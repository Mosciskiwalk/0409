...
# 创建 Firefox 数据目录
mkdir -p ~/mullvad-browser-data

# 运行 Firefox 容器
echo -e "${YELLOW}正在启动 mullvad-browser 容器...${RESET}"
docker rm -f mullvad-browser 2>/dev/null || true
docker run -d \
...
# 检查容器是否成功启动
if ! docker ps | grep -q mullvad-browser; then
...
pkill -f "ngrok http 5800 --name mullvad-browser" >/dev/null 2>&1 || true
nohup /usr/local/bin/ngrok http 5800 --name mullvad-browser --authtoken=${NGROK_TOKEN} >/dev/null 2>&1 &
...
echo -e "${YELLOW}注意: Ngrok 进程在后台运行，如需停止请使用 'pkill -f \"ngrok http 5800 --name mullvad-browser\"' 命令${RESET}"
